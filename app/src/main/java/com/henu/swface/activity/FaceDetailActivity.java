		...
		public void handleMessage(Message msg) {
			switch (msg.arg1) {
				...
				case FinalUtil.UPDATE_DETAIL_IO_EXCEPTION:
					...
					Toast.makeText(getApplicationContext(), "出现异常，重命名失败", Toast.LENGTH_LONG).show();
					break;
				case FinalUtil.REMOVE_FACE_IO_EXCEPTION:
					dialog.dismiss();
					Log.i(TAG, "handleMessage: test");
					showNormalDialog("温馨提示", "删除失败，请检查!!!", true, null, true);
					break;
				case FinalUtil.REMOVE_FACE_BMOB_EXCEPTION:
					dialog.dismiss();
					showNormalDialog("温馨提示", "删除失败，发生了一些未知错误，请重试", false, null, true);
					break;
...
//					showNormalDialog(null, "恭喜，删除人脸成功！", true, null, true);
				...
				case FinalUtil.DETECT_FAILED_IO_EXCEPTION:
					dialog.dismiss();
					showNormalDialog("温馨提示","连接失败，请稍后重试",true,null,true);
					break;
